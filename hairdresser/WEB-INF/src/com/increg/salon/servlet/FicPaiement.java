...
public void performTask(
	...
	try {
		if (Action == null) {
			...
			request.setAttribute("Etat", "Memoire");
	        		...
	        		|| (Action.equals("Impression") 
                        && ((CD_PAIEMENT == null) || (CD_PAIEMENT.length() == 0) || (CD_PAIEMENT.equals("0"))))) {
		...
		else if (((Action.equals("Modification")) && (CD_MOD_REGL == null)) 
	...
	if ((Action == null) || (!Action.equals("Impression"))) {
	    String reqSQL = "select * from FACT, CLI where FACT.FACT_HISTO = 'N' and FACT.CD_CLI=CLI.CD_CLI and (CD_PAIEMENT is null or CD_PAIEMENT=" + aPaiement.getCD_PAIEMENT() + ") order by FACT.DT_CREAT, NOM, PRENOM";
		...
		// Création en mémoire d'une facture fictive contenant toutes les lignes
		FactBean aFact = null;
