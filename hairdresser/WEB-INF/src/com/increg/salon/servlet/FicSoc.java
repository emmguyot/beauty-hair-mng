...
public void performTask(HttpServletRequest request, HttpServletResponse response) {
		...
		if (Action == null) {
			...
			try {
                if (mySalon.checkLicence(aSoc)) {
                    ...
                    mySalon.setMessage("Erreur", "Cette modification n'est pas autorisée car vous n'avez pas la licence correspondante.");
                }
			}
			catch (Exception e) {
	...
	try {
		...
		getServletConfig().getServletContext().getRequestDispatcher("/ficSoc.jsp").forward(request, response);

	}
	catch (Exception e) {
		System.out.println("FicSoc::performTask : Erreur à la redirection : " + e.toString());
	}
}
}
