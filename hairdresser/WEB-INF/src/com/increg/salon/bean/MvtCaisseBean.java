...
/**
 * Insert the method's description here.
 * Creation date: (25/09/2001 21:19:11)
 * @return java.lang.String
 */
...
/**
 * Création d'un Bean Mouvement de caisse à partir de sa clé
...
/**
 * Création d'un Bean Mouvement de caisse à partir d'une date : Le dernier avant la date
 * Creation date: 8 juil. 02
 * @param dbConnect com.increg.salon.bean.DBSession
 * @param CD_MOD_REGL java.lang.String
 * @param DT_MVT Date du mouvement
 * @return Mouvement associé
...
/**
 * Obtention des mouvements de caisse lié à un paiement
 * Creation date: 6 juil. 02
 * @param dbConnect com.increg.salon.bean.DBSession
 * @param CD_PAIEMENT Paiement associé
...
public void maj(DBSession dbConnect) throws SQLException {
    ...
    StringBuffer colonne = new StringBuffer("");
    StringBuffer where = new StringBuffer(" where CD_MOD_REGL=" + Integer.toString(CD_MOD_REGL)
                            + " and DT_MVT=" + DBSession.quoteWith(formatDate.formatEG(DT_MVT.getTime()), '\''));
    ...
    /**
     * Pas de mise à jour de la caisse !!!
     * Doit être fait si besoin en dehors
     */
...
/**
 * Calcul le solde de caisse à l'issue du mouvement en fonction du solde de départ et de la quantité
 * @param dbConnect Connexion à la base à utiliser
 * @param CD_TYP_MCA Code de la caisse
 * @param soldeAvant Solde avant le mouvement
 ...
 * @param dbConnect Connexion à la base à utiliser
 * @param qte java.math.BigDecimal
 * @throws SQLException En cas d'erreur durant la mise à jour
 */
 ...
 * Insert the method's description here.
 * Creation date: (25/09/2001 21:19:11)
 * @param newDEVISE java.lang.String
 */
 ...
 * Insert the method's description here.
 * Creation date: (16/09/2001 19:31:27)
 * @param newSOLDE_AVANT java.math.BigDecimal
 */
...
public static boolean checkAndFix (DBSession dbConnect, String CD_MOD_REGL, String DT_DEBUT) throws SQLException {
    ...
    String reqSQL = "select * from MVT_CAISSE where CD_MOD_REGL in (" + CD_MOD_REGL + modReglSup + ")"
            + " and DT_MVT >= " + DBSession.quoteWith(DT_DEBUT, '\'')
            + " order by DT_MVT ASC";
    ...
    public static int purge(DBSession dbConnect, java.util.Date dateLimite) throws FctlException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String reqSQLSolde = "select * from MVT_CAISSE where DT_MVT < " + DBSession.quoteWith(formatDate.format(dateLimite), '\'')
                                + " and CD_MOD_REGL = " + aCaisse.getCD_MOD_REGL()
                                + " order by DT_MVT desc"
                                + " limit 1";
